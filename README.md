...
Every sample here runs the *shipping* Cephable assistant: the same agent loop, system prompt, ~55 built-in tools, AI Skills, MCP servers, and local llama.cpp model that the Cephable app itself uses. Nothing in these samples sends your prompts or your data to a cloud model — inference and tool execution happen on the machine.
...
| **[nextjs-vercel-ai-ui](samples/nextjs-vercel-ai-ui)** | TypeScript · Next.js · Vercel AI SDK | A chat UI built with the Vercel AI SDK, streaming Cephable's **real agent steps and tool calls** into the browser as they happen. Two of its tools draw straight into the page — the agent decides to render a timeline, and one appears. |
...
All four samples need the same two things. Full detail in **[docs/getting-started.md](docs/getting-started.md)**.
...
2. **No streaming.** `POST /v1/runs` blocks until the run reaches a terminal state — tens of seconds to minutes for real agent work. Set client timeouts *above* the server's `timeoutMs`, not below.
